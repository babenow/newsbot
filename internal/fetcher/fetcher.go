...
import (
	...
	"github.com/babenow/newsbot/internal/source"
)
...
func (f *Fetcher) Fetch(ctx context.Context) error {
	...
	for _, src := range sources {
		...
		go func(source Source) {
			...
			if err != nil {
				log.Printf("[ERROR] Fetching items from sourse %s: %v", source.Name(), err)
				return
			}
...
func (f *Fetcher) processItems(ctx context.Context, source Source, items []model.Item) error {
	for _, item := range items {
		...
		if f.itemShoulBeSkipped(item) {
			continue
		}

		if err := f.articleStorage.Store(ctx, model.Article{
			SourceID:    source.ID(),
			Title:       item.Title,
			Link:        item.Link,
...
func (f *Fetcher) itemShoulBeSkipped(item model.Item) bool {
	for _, keyword := range f.filterKeywords {
		titleContainsKeyword := strings.Contains(strings.ToLower(item.Title), keyword)
		for _, category := range item.Categories {
			if category == keyword || titleContainsKeyword {
				return true
			}
		}
	}
